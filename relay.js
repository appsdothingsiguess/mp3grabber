...
import { createWriteStream, unlink, existsSync, mkdirSync, writeFileSync, readdirSync, statSync, readFileSync, unlinkSync } from 'fs';
...
function transcribe(file, forceCPU = false) {
    try {
        ...
        try {
            ...
            // FIRST: Always try to recover from result file for exit code 3221226505
            if (execError.status === 3221226505) {
                const resultPath = file + '.result.json';
                console.log(`⚠️  Windows CUDA crash detected (exit ${execError.status}), attempting recovery...`);
                
                // Give filesystem a moment to flush (Windows disk I/O delay)
                // Use sync wait to ensure file is written
                try {
                    ...
                    for (let i = 0; i < 5; i++) {
                        if (existsSync(resultPath)) {
                            ...
                            if (recovered && recovered.success && typeof recovered.transcript === 'string') {
                                ...
                                console.log(`✅ Transcription complete! (recovered after Windows crash)`);
            ...
            // THEN: Check for CUDA errors and retry with CPU (existing logic)
            const errorOutput = (execError.stderr || execError.stdout || execError.message || '').toString();
            const hasCudaKeyword = errorOutput.includes('cudnn') || 
            ...
            const looksLikeError = /Traceback|Error:|Exception|failed|AssertionError|exit code|MODULE_NOT_FOUND/i.test(errorOutput) ||
                                  execError.status === 3221226505; // Windows access violation (DLL error)
            const isCudaError = hasCudaKeyword && (looksLikeError || execError.status === 3221226505);
        ...
        if (jsonResult.success) {
            const resultPath = file + '.result.json';
            if (existsSync(resultPath)) { try { unlink(resultPath, () => {}); } catch (_) {} }
...
if (process.platform === 'win32') {
  try {
    ...
    if (pathsToAdd.length > 0) {
      ...
      console.warn('   Install: pip install nvidia-cublas nvidia-cudnn-cu13');
    }
  } catch (pathError) {
